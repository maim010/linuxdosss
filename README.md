...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-8.2-orange.svg" alt="Version">
</p>
...
- ✨ **爬楼模式**：智能楼层计数器，精确追踪阅读进度
- 🎛️ **功能开关**：可独立控制自动点赞、自动回复、等待延迟
- ⏱️ **倒计时显示**：实时显示剩余帖子数/时间和已用时间
- 🎯 **运行模式**：支持无尽模式、帖子数量限制、时间限制三种模式
- 📊 **增强托盘**：托盘图标显示详细运行信息（模式、倒计时、统计）
- 🖥️ **优化界面**：调整窗口尺寸（550x920），更紧凑美观
...
| 功能 | 描述 |
|------|------|
| **爬楼模式** | 使用楼层计数器精确追踪阅读进度，支持宽窗口和窄窗口两种显示格式 |
...
| **等级追踪** | 实时获取用户等级和升级进度 |
| **进度统计** | 详细统计浏览、爬楼、点赞、回复数量 |
| **系统托盘** | 支持最小化到托盘，显示详细运行信息 |

### 运行模式

- **无尽模式**：持续运行直到手动停止
- **帖子数量模式**：达到指定帖子数后自动停止（带倒计数）
- **时间限制模式**：达到指定时间后自动停止（带倒计时）
...
- **现代化 UI**：深色主题设计，自定义标题栏，紧凑布局（550x920）
- **实时日志**：详细显示运行状态和操作记录
- **倒计时显示**：停止按钮旁显示剩余帖子数/时间和已用时间
- **进度面板**：可视化展示升级进度和各项指标
